...
export class PlaywrightScraper implements WebScraper {
  ...
  async extractImages(url: string): Promise<ScrapeResult> {
    // Re-launch a fresh browser on each retry — reusing a crashed/timed-out
    // browser context produces inconsistent results.
    return withRetry(
      async () => {
        let browser;
        try {
      ...
      { label: "playwright.scrape", retries: 2, baseDelayMs: 2000, maxDelayMs: 10000 },
    );
  }
}
